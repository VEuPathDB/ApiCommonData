package ApiCommonData::Load::Plugin::InsertPolyAGenes;
...
sub run {
  ...
  my $fullName = $self->getArg('sampleName');

  my $tempTableCreateHSql = "  CREATE TABLE apidb.PolyAGeneCoordinates_$sampleName AS
...
WHERE na_sequence_id in 
 (SELECT distinct na_sequence_id FROM apidb.splicesitefeature where type = 'Poly A' and sample_name='$sampleName')
ORDER BY na_sequence_id, alpha";
  ...
  my $sql ="select ssf.splice_site_feature_id, ssf.location, ssf.strand, ga.source_id, abs(ga.gamma-ssf.location) as dist_to_cds,
              ...
              from apidb.splicesitefeature ssf, apidb.PolyAGeneCoordinates_$sampleName ga
  ...
  my $numRow=0;
  whlie (my ($splice_site_feature_id, $location, $strand, $source_id, $dist_to_cds, $within_cds, $sample_name,$count, $count_per_million, $avg_mismatches, $is_unique, $type, $na_sequence_id, $external_database_release_id)) = $sth->fetchrow_array(){
   my $polyAGenes =  GUS::Model::ApiDB::PolyAGenes->new({'splice_site_feature_id' => $splice_site_feature_id,
						    'location' => $location,
						    'strand' => $strand,
   ...
   $numRow++;
    if ($numRow % 100 == 0){
      $self->log("$count rows added to apidb.PolyAGenes.");
      $self->undefPointerCache();
    }
  }
  my $tempTableDropH = $dbh->prepare(<<SQL);
   DROP TABLE apidb.PolyAGeneCoordinates_$sampleName
  SQL
