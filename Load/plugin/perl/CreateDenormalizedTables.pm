...
The following values are passed in to the psql as macro values:
 ...
 - :FILTER - taxon_id, if organism specific
...
sub run {
  ...
  $psqlFile =~ /(.+).psql/ or $error("psqlFile name must be in form MY_DENORM_TABLE.psql");
  my $tableName = $1;
  ...
  foreach my $sql (@sqlList) {
    ...
    $newSql =~ s/\:FILTER/$taxonId/g;
    $newSql =~ $mode eq 'create'?
      s/\:CREATE_OR_INSERT/'CREATE TABLE $schema.$tableName AS '/g :
      s/\:CREATE_OR_INSERT/'INSERT INTO $schema.$tableName   '/g;
...
sub undoPreprocess {
  ...
  if ($mode eq 'insert') {
    ...
    $dbh->do($sql) || $error->("Failed executing $sql");
