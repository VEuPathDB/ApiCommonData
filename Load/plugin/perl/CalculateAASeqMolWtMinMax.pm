package ApiCommonData::Load::Plugin::CalculateAASeqMolWtMinMax;
...
my $argsDeclaration =
  [
   ...
   stringArg({ name => 'seqTable',
	       descr => 'where to find the target AA sequences in the form DoTS.table',
	       constraintFunc => undef,
	       isList => 0,
	       reqd => 1,
	     })
...
my $newSeqAttr = GUS::Model::ApiDB::AaSequenceAttribute->
    new({aa_sequence_id => $aaSeqId});
