...
sub run {
    ...
    while (my $line = <$FH>) {
        ...
        my $newRow = GUS::Model::DoTS::AASequenceEnzymeClass->new({
            ...
            domain_score           => sprintf("%.4f", $confidence_score),
            length_score           => sprintf("%.4f", $length_vs_cluster_mean),
            length_mean            => int($cluster_mean_length + 0.5),
            num_supporting_cluster => int($n_supporting),
            num_protein_cluster    => int($cluster_size),
            num_any_ec_cluster     => int($n_annotated_in_cluster),
            num_supporting_group   => int($n_supporting_in_group),
            num_protein_group      => int($group_size),
            num_any_ec_group       => int($n_annotated_in_group),
        });
