  ...
  my $argsDeclaration  =
    [
     ...
     fileArg({name => 'resultFileDir',
	      descr => 'Directory where multiple TSV files of interpro results reside',
...
sub run {
  ...
  my @resultFiles = grep { /\.tsv$/ } readdir RESULTSDIR;
  closedir RESULTSDIR;
  ...
  $self->log("Processing ".scalar(@resultFiles)." Interproscan tsv result files");
  ...
  foreach my $file (@resultFiles) {
    ...
    chomp;
    my @myArray = split(/\t/, $_);
    $self->processProteinResults(\@myArray);
...
sub processProteinResults {
  ...
  my $aaId = $self->sourceId2aaSeqId($proteins->[0]);
