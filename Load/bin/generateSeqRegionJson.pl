...
if ($ncbiTaxId) {
  $sql = "select source_id, SEQUENCE_TYPE, LENGTH from webready.GenomicSeqAttributes_p where ncbi_tax_id = $ncbiTaxId and is_top_level = 1";
}
...
where:
  ...
  --gusConfigFile: required, it should point to a inc- instance due to query webready.GenomicSeqAttributes_p table
