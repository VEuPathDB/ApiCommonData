...
foreach my $key ( keys(%$appNodeCategories) ) {
  ...
  for (my $i=0; $i<$categoryCount; $i++) {
    ...
    my ($desc,$type,$displayName) = '';
    my $ot_source_id = $ot_source_ids->[$i];
 
    unless($ot_source_id =~/^\W$/) {
      $descRow->execute($ot_source_id);
      $desc = $descRow->fetchrow_array();
      $typeRow->execute($ot_source_id);
      $type = $typeRow->fetchrow_array();
      $displayRow->execute($ot_source_id);
      $displayName = $displayRow->fetchrow_array();
     }
