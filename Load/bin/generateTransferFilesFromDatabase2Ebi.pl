...
foreach my $abbrev (sort keys %isAnnotated) {
  ...
  my $makeGenomeFastaCmd = "gusExtractSequences --outputFile $dnaFastaFile --gusConfigFile $gusConfigFile --idSQL 'select s.source_id, s.SEQUENCE from webready.GenomicSeqAttributes_p sa, dots.nasequence s where s.na_sequence_id = sa.na_sequence_id and sa.is_top_level = 1 and sa.NCBI_TAX_ID=$ncbiTaxonId'";
