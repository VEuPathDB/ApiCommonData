...
## to populate apidb.spliceSiteGenes table
...
my $tempTableCreateH = $dbh->prepare(<<SQL);
  ...
  (select min(coding_start) as coding_start_min, na_sequence_id from webready.GeneAttributes_p
   where gene_type='protein coding'
   group by na_sequence_id)  sub,
  (select min(nxt.coding_start) as delta, ga.source_id
   from webready.GeneAttributes_p ga, webready.GeneAttributes_p nxt
...
-- CASE B: other genes on forward strand
 ...
 FROM webready.GeneAttributes_p ga, 
   (select max(prev.coding_end) as alpha, ga.source_id
    from webready.GeneAttributes_p ga, webready.GeneAttributes_p prev
...
--CASE C:  last gene on reverse strand
 SELECT ga.na_sequence_id , ga.source_id, ga.start_min as alpha, ga.coding_start as beta, sa.length as gamma,
  CASE WHEN (sub3.delta < sub4.delta) THEN sub3.delta ELSE sub4.delta END AS delta, ga.strand
 FROM webready.GeneAttributes_p ga, webready.GenomicSeqAttributes_p sa,
    (select max(end_max) as max_end_max, na_sequence_id from webready.GeneAttributes_p
     where gene_type='protein coding'
     group by na_sequence_id)  sub,
...
WHERE na_sequence_id in 
 (SELECT distinct na_sequence_id FROM apidb.splicesitefeature where type = 'Splice Site')
...
my $insertH = $dbh->prepare(<<SQL);
 INSERT INTO apidb.SpliceSiteGenes
...
print STDERR "Create Splice Site Genes table\n" if $verbose;
