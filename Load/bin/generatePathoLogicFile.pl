...
sub printFasta {
  my ($dbh, $chr, $path)  = @_;
  my $sql = "select sequence from webready.GenomicSequenceSequence_p where source_id = '$chr'"; 
...
sub printGeneticElement {
  ...
  my $sql = "select distinct ga.CHROMOSOME, ga.SEQUENCE_ID from webready.GeneAttributes_p ga where ga.organism = '$organism' and ga.chromosome is not null order by ga.chromosome";
...
EOL
  ...
  while(my $arr = $sth->fetchrow_arrayref()){
    ...
    my $sqlGO = "SELECT distinct gts.go_id, gts.go_term_name, gts.evidence_code FROM webready.GoTermSummary_p gts where gts.source_id = '$id' ORDER BY gts.go_id";
