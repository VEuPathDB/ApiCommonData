...
select -- virtual feature locations mapped through SequencePiece
       case
         when subclass_view = 'GeneFeature'
...
CREATE TABLE apidb.AssemblyAttributes&1 AS
SELECT a.source_id,
       CASE
         WHEN SUBSTR(tn.name, 1, 6) = 'Crypto'
           THEN 'CryptoDB'
...
-------------------------------------------------------------------------------
CREATE TABLE apidb.IsolateAttributes&1 AS 
...
-------------------------------------------------------------------------------
CREATE TABLE apidb.SageTagAttributes&1 AS
SELECT CASE
         WHEN SUBSTR(tn.name, 1, 6) = 'Crypto'
           THEN 'CryptoDB'
         WHEN SUBSTR(tn.name, 1, 6) = 'Plasmo'
...
DROP TABLE apidb.AsmAlignmentGene;

CREATE TABLE apidb.AsmAlignmentGene AS
...
DROP TABLE apidb.AsmAlignmentNoGene;

CREATE TABLE apidb.AsmAlignmentNoGene AS
...
CREATE TABLE AsmAlignmentGeneSummary&1 AS
