...
--   (SELECT max(c.content) AS comment_string, c.stable_id
----    FROM @COMMENT_SCHEMA@comments@COMMENT_DBLINK@ c
--      FROM comments.comments@apidb.login_comment c
--      FROM toxocomm.comments@apidb.login_comment c
--      WHERE c.review_status_id = 'task'
--      GROUP BY c.stable_id) cmnt,
     (SELECT source_id AS comment_string, source_id AS stable_id
...
WHERE gf.na_feature_id = nl.na_feature_id
  ...
  AND gf.source_id = cmnt.stable_id(+)
  AND gf.source_id = etc.source_id(+)
--  AND gf.source_id
--      IN ('PF11_0344', 'MAL13P1.1', 'MAL13P1.106', 'MAL13P1.114',
--          '1399.pretRNA-Leu-1', '2277.pre-tRNA-Glu-1', 'MAL13_5.8SrRNA',
--          'Pv000020', 'MALPY00014_tRNA_Asn1', 'PY00039')
;
...
---------------------------
-- do it again, making tables this time
-- (workaround for Oracle bug)
-- start with genes (again)
---------------------------
