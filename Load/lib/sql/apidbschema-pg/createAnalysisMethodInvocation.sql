...
SELECT nextval('core.tableinfo_sq'), 'AnalysisMethodInvocation',
       ...
       (SELECT database_id as core_database_id FROM core.DatabaseInfo WHERE lower(name) = 'apidb'), 
       0, 0,  NULL, NULL, 1,current_timestamp, 1, 1, 1, 1, 1, 1, 1, 1,
       (SELECT MAX(project_id) FROM core.ProjectInfo), 0
WHERE 'analysismethodinvocation' NOT IN (SELECT lower(name) FROM core.TableInfo
    WHERE database_id = (SELECT database_id as core_database_id FROM core.DatabaseInfo WHERE lower(name) = 'apidb'));
