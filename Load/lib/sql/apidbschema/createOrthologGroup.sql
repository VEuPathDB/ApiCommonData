CREATE TABLE apidb.OrthologGroup (
 ...
 number_of_members            NUMBER(12),
 number_of_core_members       NUMBER(12),
 number_of_peripheral_members NUMBER(12),
...
CREATE INDEX apidb.og_name_ix ON apidb.OrthologGroup (group_id, ortholog_group_id) tablespace indx;
CREATE INDEX apidb.og_core_ix ON apidb.OrthologGroup (is_residual, ortholog_group_id, group_id) tablespace indx;
...
CREATE TABLE apidb.OrthologGroupStats (
 ortholog_group_stat_id       NUMBER(10) NOT NULL,
 group_id                     VARCHAR2(12) NOT NULL,
 stat_type                    VARCHAR2(10) NOT NULL,
 evalue                       FLOAT,
 protein_subset               VARCHAR2(3) NOT NULL,
 modification_date            DATE NOT NULL,
 user_read                    NUMBER(1) NOT NULL,
 user_write                   NUMBER(1) NOT NULL,
 group_read                   NUMBER(1) NOT NULL,
...
INSERT INTO core.TableInfo
    (table_id, name, table_type, primary_key_column, database_id, is_versioned,
     ...
     other_read, other_write, row_user_id, row_group_id, row_project_id,
     row_alg_invocation_id)
SELECT core.tableinfo_sq.nextval, 'OrthologGroupStats',
       'Standard', 'ortholog_group_stat_id',
       d.database_id, 0, 0, '', '', 1, sysdate, 1, 1, 1, 1, 1, 1, 1, 1,
       p.project_id, 0
FROM dual,
     (SELECT MAX(project_id) AS project_id FROM core.ProjectInfo) p,
     (SELECT database_id FROM core.DatabaseInfo WHERE name = 'ApiDB') d
WHERE 'OrthologGroupStats' NOT IN (SELECT name FROM core.TableInfo
                                 WHERE database_id = d.database_id);
...
INSERT INTO core.TableInfo
    (table_id, name, table_type, primary_key_column, database_id, is_versioned,
     ...
     other_read, other_write, row_user_id, row_group_id, row_project_id,
     row_alg_invocation_id)
SELECT core.tableinfo_sq.nextval, 'OrthoAASequence',
       'Standard', 'AA_SEQUENCE_ID',
       d.database_id, 0, 1, s.table_id, '', 1,sysdate, 1, 1, 1, 1, 1, 1, 1, 1,
       p.project_id, 0
FROM dual,
     (SELECT MAX(project_id) AS project_id FROM core.ProjectInfo) p,
...
WHERE 'orthoaasequence' NOT IN (SELECT lower(name) FROM core.TableInfo
                                    where database_id = d.database_id);
...
ON apidb.OrthologGroupAASequence (group_id, aa_sequence_id) tablespace indx;

CREATE INDEX apidb.ogas_asog_ix
ON apidb.OrthologGroupAASequence (aa_sequence_id, group_id) tablespace indx;
------------------------------------------------------------------------------
