...
DROP VIEW dotsVer.IsolateFeatureVer;

CREATE VIEW dotsVer.IsolateFeatureVer as 
SELECT NA_Feature_ID, 
 NA_SEQUENCE_ID, 
 SUBCLASS_VIEW, 
 NAME, 
...
GRANT INSERT, SELECT, UPDATE, DELETE ON dotsVer.IsolateFeatureVer TO gus_w;
GRANT SELECT ON dotsVer.IsolateFeatureVer TO gus_r;

------------------------------------------------------------------------------

DELETE core.TableInfo where name='IsolateFeatureVer';
...
INSERT INTO core.TableInfo
    (table_id, name, table_type, primary_key_column, database_id, is_versioned,
     ...
     other_read, other_write, row_user_id, row_group_id, row_project_id, 
     row_alg_invocation_id)
SELECT core.tableinfo_sq.nextval, 'IsolateFeatureVer',
...
FROM dual,
     ...
     (SELECT table_id FROM core.TableInfo WHERE lower(name) = 'nafeatureimp') v,
     (SELECT table_id FROM core.TableInfo WHERE lower(name) = 'nafeature') s
WHERE 'IsolateFeatureVer' NOT IN (SELECT name FROM core.TableInfo
                                    WHERE database_id = d.database_id);
