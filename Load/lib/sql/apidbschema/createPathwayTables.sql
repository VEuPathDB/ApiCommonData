...
CREATE TABLE ApiDB.PathwayRelationship (
  ...
  IS_REVERSIBLE           NUMBER(1)    NOT NULL,
  MODIFICATION_DATE       DATE,
  USER_READ               NUMBER(1),
  USER_WRITE              NUMBER(1),
  GROUP_READ              NUMBER(1),
...
CREATE TABLE ApiDB.ReactionRelationship (
  ...
  FOREIGN KEY (PATHWAY_RELATIONSHIP_ID) REFERENCES ApiDB.PathwayRelationship (PATHWAY_RELATIONSHIP_ID)
);
...
CREATE INDEX apidb.pathwayNode_idx ON ApiDB.PathwayNode (PATHWAY_NODE_ID,SOURCE_ID);
