...
my $soTerms = { 'coding_gene'=>'protein_coding',
		...
		'RNase_MRP_RNA_gene'=> 'RNase_MRP_RNA',
                ...
                'telomerase_RNA_gene' => 'telomerase_RNA',
...
sub makeGusGene {
  ...
  $plugin->error("Trying to make gus skeleton from a tree rooted with an unexpected type: '$type'") 
     unless (grep {$type eq $_} ("haplotype_block","coding_gene", "tRNA_gene", "rRNA_gene", "snRNA_gene", "snoRNA_gene", "misc_RNA_gene", "misc_feature_gene", "repeated_gene","pseudo_gene","SRP_RNA_gene","RNase_MRP_RNA_gene","RNase_P_gene","RNase_MRP_gene","RNase_P_RNA_gene","ncRNA_gene","scRNA_gene", "miRNA_gene", "transposable_element_gene","telomerase_RNA_gene"));
