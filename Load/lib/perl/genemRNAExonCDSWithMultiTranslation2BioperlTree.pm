package ApiCommonData::Load::genemRNAExonCDSWithMultiTranslation2BioperlTree;
...
sub preprocess {
    ...
    OUTER: foreach my $bioperlFeatureTree (@topSeqFeatures) {
	    ...
	    if($type eq 'protein_coding_gene' || $type eq 'ncRNA_gene'){
	    ...
	    if($type eq 'tRNA' || $type eq 'rRNA' || $type eq 'snRNA' || $type eq 'snoRNA') {
	      ...
	      my($geneID) = ($geneFeature->has_tag('ID')) ? $geneFeature->get_tag_values('ID') : die "ERROR: missing ID for ${type}_gene at $geneLoc->start(), $geneLoc->end()\n";
