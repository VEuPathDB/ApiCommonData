...
";
  if($overrideOnly){
    $sql = "select s.name
                  ...
                  , json_query(os.annotation_properties, '\$.hidden') as hidden -- gives json array
                  , json_value(os.annotation_properties, '\$.displayOrder[0]') as display_order
                  , json_value(os.annotation_properties, '\$.defaultDisplayRangeMin[0]') as display_range_min
                  , json_value(os.annotation_properties, '\$.defaultDisplayRangeMax[0]') as display_range_max
                  , json_value(annotation_properties, '\$.defaultBinWidth[0]') as bin_width_override
                  , case when lower(json_value(os.annotation_properties, '\$.is_temporal[0]')) = 'yes' then 1 else 0 end as is_temporal
                  , case when lower(json_value(os.annotation_properties, '\$.is_featured[0]')) = 'yes' then 1 else 0 end as is_featured
                  , case when lower(json_value(os.annotation_properties, '\$.repeated[0]')) = 'yes' then 1 else 0 end as is_repeated
                  , case when lower(json_value(os.annotation_properties, '\$.mergeKey[0]')) = 'yes' then 1 else 0 end as is_merge_key
                  , case when lower(json_value(os.annotation_properties, '\$.impute_zero[0]')) = 'yes' then 1 else 0 end as impute_zero
                  , json_query(os.annotation_properties, '\$.variable') as provider_label -- gives json array
                  , os.ordinal_values as ordinal_values --gives json array
from  sres.ontologyterm s
   ,  sres.ontologysynonym os
where 
 s.ontology_term_id = os.ontology_term_id
