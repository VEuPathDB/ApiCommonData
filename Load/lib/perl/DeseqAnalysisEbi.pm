...
sub munge {
    ...
    foreach my $d (sort @ds) {
        next unless -e "$d/genes.htseq-union$append.counts";
	    my $sample = $d;
    ...
    if ($@) {
        print "Coverage is not deep enough to run DESeq2. Differential expression will not be loaded for this contrast.\n";
    }
    else {
