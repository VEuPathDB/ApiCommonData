...
sub traverseSeqFeatures {
	...
	my $type = $RNA->primary_tag;
	   ...
	   # print STDERR "-----------------$type----------------------\n";
	    ...
	    my($geneID) = $geneFeature->get_tag_values('Name');
...
sub copyQualifiers {
  ...
  for my $qualifier ($geneFeature->get_all_tags()) {
    ...
    if ($bioperlFeatureTree->has_tag($qualifier) && $qualifier ne "Name" && $qualifier ne "Parent" && $qualifier ne "Derives_from") {
    ...
    } elsif($qualifier ne "Name" && $qualifier ne "Parent" && $qualifier ne "Derives_from") {
      $bioperlFeatureTree->add_tag_value(
                             $qualifier,
                             $geneFeature->get_tag_values($qualifier)
                           );
