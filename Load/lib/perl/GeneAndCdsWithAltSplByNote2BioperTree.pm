package ApiCommonData::Load::GeneAndCdsWithAltSplByNote2BioperTree;

## genbank file with alternative split gene that can not seperate by split site,
## add new resolver_method based on product name
## this requires that CDS and mRNA have exact same product
...
#    my $unflattener = Bio::SeqFeature::Tools::Unflattener->new;
	...
	$unflattener->unflatten_seq(-seq=>$bioperlSeq,
					...
					my @trnames = map {/encoded by transcript\s+(\S+)/; $1} @notes;
					@trnames = grep {$_} @trnames;
					my $trname;
					if (@trnames == 0) {
					  $self->throw("UNRESOLVABLE at unflattener_seq at $cids[0]");
					} elsif (@trnames == 1) {
					  $trname = $trnames[0];
					} else {
					  $self->throw("AMBIGUOUS at unflattener_seq: $trnames[0]");
					}
					my @container_sfs = grep { my ($cnote) = 
								     $_->has_tag('note') ?
								       $_->get_tag_values('note') :
									 ('');
								   $cnote =~ s/transcript //;
								   $trname =~ s/;$//;
								   $cnote eq $trname;
					...
					if (@container_sfs == 0) {
					  $self->throw("UNRESOLVABLE at unflattener_seq container at $cids[0]");
					} elsif (@container_sfs == 1) {
					  # we got it!
					  return $container_sfs[0];
					} else {
					  $self->throw("AMBIGUOUS at unflattener_seq container at $cids[0]");
					}
                                      }
                                    },
                                 -use_magic=>1);
	...
	OUTER: foreach my $bioperlFeatureTree (@topSeqFeatures) {
		...
		## for those genes with framshift in note tag, load as pseudogene, such as cimmH538-4
		if (!$geneFeature->get_SeqFeatures && $geneFeature->has_tag("note") && !$geneFeature->has_tag("pseudo")) {
		  my ($note) = $geneFeature->get_tag_values("note");
		  if ( $note =~ /frameshift/i || $note =~ /frame shift/i) {
		    $geneFeature->add_tag_value("pseudo", "") if (!$geneFeature->has_tag("pseudo") );
		  }
		}
...
#    foreach my $RNA (@RNAs){ 
    foreach my $RNA (sort {$a->start <=> $b->start || $a->end <=> $b->end } @RNAs){
