package ApiCommonData::Load::GusSkeletonMakerSpecial;
...
#	print ERROR "exonCdsArray start = $exonCdsArray->[1], end =$exonCdsArray->[2]\n";
  ...
  foreach my $gusTranscript (@gusTranscripts) {
    ...
    if ( ($gusGene->getName() eq 'coding_gene'
	 ...
	 && $gusTranscript->getName() eq 'mRNA') {
      ...
      my $ct = @translatedAaFeatures;
      print STDERR "For transcriptId $transcriptId, there are total translatedAaFeatures: $ct\n";
