package ApiCommonData::Load::GusSkeletonMakerWithMultiTranslation;
...
sub makeGeneSkeleton{
  ...
  foreach my $bioperlTranscript ($bioperlGene->get_SeqFeatures()) {
	...
	$bioperlExon->{gusFeature} = $gusExon;
      ...
      ## Skip for duplicate transcripts: RNAFeatureExon rows already exist from the first
      ## mRNA iteration. Re-adding them would (a) create duplicate in-memory children that
      ## cause getExons() to return doubled exon lists, corrupting the exonPath key used by
      ## the postprocessing lookup, and (b) violate the unique constraint on DoTS.RNAFeatureExon
      ## with --commit.
      unless ($isDuplicateTranscript) {
...
#      ($codingEnd) ? $rnaFeatureExon->setCodingEnd($codingEnd) : $rnaFeatureExon->setCodingEnd('');
    ...
    if ( ($bioperlGene->primary_tag() eq 'coding_gene'
	 ...
	 && ($bioperlTranscript->primary_tag() eq 'mRNA'
      ...
      ## Build a CDS signature for this transcript: sorted "codingStart_codingEnd" pairs.
      ## For duplicate transcripts (same exon structure but different CDS), we only add a
      ## new TranslatedAAFeat/Seq when the CDS signature is not already recorded on this
      ## gusTranscript.  This prevents duplicate DB rows (--commit failure) while correctly
      ## loading one TranslatedAA per distinct translation for the shared transcript.
      my $cdsSignature = join(",", map { ($_->[1] // '') . "_" . ($_->[2] // '') }
                                   sort { ($a->[1] // '') cmp ($b->[1] // '') ||
                                          ($a->[2] // '') cmp ($b->[2] // '') }
                                   @gusExonsAndCodingLocations);

      if ($isDuplicateTranscript) {
        if (exists $gusTranscript->{_cdsSignatures}{$cdsSignature}) {
          ## Truly identical translation already recorded -- nothing new to add.
          next;
        }
      }
      ## Record this CDS signature so subsequent duplicate iterations can detect it.
      $gusTranscript->{_cdsSignatures}{$cdsSignature} = 1;
...
sub printTranscriptIds {
  ...
  foreach my $gusTranscript (@gusTranscripts) {
    ...
    ## Deduplicate order numbers: old code without our fix created duplicate
    ## RNAFeatureExon objects for structurally-duplicate transcripts, causing
    ## getExons() to return each exon twice and yielding paths like "1,1,2,2,...".
    ## Deduplicate here so the written key is always canonical "1,2,...,N".
    { my %seen; @exonPath = grep { !$seen{$_}++ } sort {$a <=> $b} @exonPath; }
...
sub printTranscriptInfo {
  ...
  foreach my $gusTranscript (@gusTranscripts) {
    ...
    ## Deduplicate order numbers for the same reason as in printTranscriptIds.
    { my %seen; @exonPath = grep { !$seen{$_}++ } sort {$a <=> $b} @exonPath; }
    my $exonPathStr = join (",", @exonPath);
...
sub setTranscriptIds{
  ...
  foreach my $gusTranscript (@gusTranscripts) {
    ...
    my $rawTranscriptSeq  = $gusTranscript->getFeatureSequence();
    (my $transcriptSeq = $rawTranscriptSeq) =~ s/[^ATCG]/N/gi;

    ## get exonPath, deduplicating order numbers in case the transcriptInfoAndIds file was
    ## written by an older version of the code that produced duplicate RNAFeatureExon objects
    ## (resulting in repeated order numbers like "1,1,2,2,...").
    my @ePaths;
    ...
    if ( ($gusGene->getName() eq 'coding_gene'
	 ...
	 && $gusTranscript->getName() eq 'mRNA') {
      ...
      my $aaCount = 0;
      ## Sort by the minimum coding_start of each TranslatedAAFeature's AAFeatureExon children.
      ## translation_stop is a DB column that is not yet populated in memory before commit,
      ## so it cannot be used as a sort key here.  The coding_start from AAFeatureExon
      ## is set when the object is constructed and gives a stable, deterministic ordering
      ## that distinguishes two different translations of the same transcript.
      foreach my $translatedAaFeature (
          sort {
            my $aMin = do { my @cs = map { $_->getCodingStart() // 0 }
                                        $a->getChildren('DoTS::AAFeatureExon', 1);
                            @cs ? (sort { $a <=> $b } @cs)[0] : 0 };
            my $bMin = do { my @cs = map { $_->getCodingStart() // 0 }
                                        $b->getChildren('DoTS::AAFeatureExon', 1);
                            @cs ? (sort { $a <=> $b } @cs)[0] : 0 };
            $aMin <=> $bMin
          } @translatedAaFeatures
      ) {
	$aaCount++;
